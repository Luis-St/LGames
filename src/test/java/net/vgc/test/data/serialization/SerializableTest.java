package net.project.test.data.serialization;

import java.io.File;
import java.nio.file.Path;

import net.project.data.serialization.Serializable;
import net.project.data.serialization.SerializationUtil;
...
@ProjectTest
public class SerializableTest implements IProjectTest {
	...
	public void start() throws Exception {
		...
		Tag.write(TestMain.resourceDir.resolve(this.deserializeConstructorPath), deserializeConstructor.serialize());
		LoadMethod loadMethod = new LoadMethod("load method", 18832015);
		LOGGER.debug("{}" , loadMethod);
		Tag.write(TestMain.resourceDir.resolve(this.loadMethodPath), loadMethod.serialize());
		DeserializeMethod deserializeMethod = new DeserializeMethod("deserialize method", 978232154);
		LOGGER.debug("{}" , deserializeMethod);
		Tag.write(TestMain.resourceDir.resolve(this.deserializeMethodPath), deserializeMethod.serialize());
		CreateMethod createMethod = new CreateMethod("create method", 135024800);
		LOGGER.debug("{}" , createMethod);
		Tag.write(TestMain.resourceDir.resolve(this.createMethodPath), createMethod.serialize());
		LoadConstructor loadConstructor = new LoadConstructor("load constructor", 96218721);
		LOGGER.debug("{}" , loadConstructor);
		Tag.write(TestMain.resourceDir.resolve(this.loadConstructorPath), loadConstructor.serialize());
	}

	@Override
	public void stop() throws Exception {
		LOGGER.debug("{}", SerializationUtil.deserialize(DeserializeConstructor.class, (CompoundTag) Tag.load(TestMain.resourceDir.resolve(this.deserializeConstructorPath))));
		LOGGER.debug("{}", SerializationUtil.deserialize(LoadMethod.class, (CompoundTag) Tag.load(TestMain.resourceDir.resolve(this.loadMethodPath))));
		LOGGER.debug("{}", SerializationUtil.deserialize(DeserializeMethod.class, (CompoundTag) Tag.load(TestMain.resourceDir.resolve(this.deserializeMethodPath))));
		LOGGER.debug("{}", SerializationUtil.deserialize(CreateMethod.class, (CompoundTag) Tag.load(TestMain.resourceDir.resolve(this.createMethodPath))));
		LOGGER.debug("{}", SerializationUtil.deserialize(LoadConstructor.class, (CompoundTag) Tag.load(TestMain.resourceDir.resolve(this.loadConstructorPath))));
	}
