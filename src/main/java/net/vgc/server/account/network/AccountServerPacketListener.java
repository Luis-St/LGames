...
public class AccountServerPacketListener extends AbstractPacketListener {
	...
	public void handleClientLogin(LoginType loginType, String name, String password) {
		...
		if (loginType == LoginType.REGISTRATION) {
			accountInfo = new PlayerAccountInfo(new InfoResult(Result.SUCCESS, TranslationKey.createAndGet("account.login.create")), agent.createUserAccount(name, password, UUID.randomUUID()));
			...
			accountInfo = new PlayerAccountInfo(new InfoResult(Result.SUCCESS, TranslationKey.createAndGet("account.login.guest")), agent.createGuestAccount(name, UUID.randomUUID()));
		} else {
			accountInfo = new PlayerAccountInfo(new InfoResult(Result.FAILED, TranslationKey.createAndGet("account.login.unknown")), PlayerAccount.UNKNOWN);
		}
